...
Below is an example of what each primitive type looks like in Java.
...
Example: you can't pour an int-full of stuff into a byte-sized container.
...
isCrazy = true; assign true to the previously-declared isCrazy
...
Every element in an array is just a variable. In other words, one of the eight primitive variable types or a reference variable.
...
Think of a reference variable as a remote control. Using the dot operator (.) on a reference variable, is like pressing a button on the remote control to access a method or instance variable. Remember, a reference variable has a value of "null" when it is not referencing any object. Arrays are always an object, even if the array is declared to hold primitives. There is no such thing as a primitive array, only an array that "hold"s primitives.
